...
| §9.4 "flagged auto-checks render as amber callouts; never approve past a missing required item" | **Not built — R-3** (core-service #7, the checklist engine, is unbuilt). The approval screen ships without the flag callouts and without the missing-item block; the approve button is unguarded by checklist state this increment | scope fence: the gate cannot exist before the checklist does |
