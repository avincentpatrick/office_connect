...
> **Resume Office-Connect — Stage C next increment: R-2 the per-diem computation engine (pure backend, on the seeded EO 77 clusters + config) OR the first React/Vite/Tailwind shell OR R-4-app (author the reimbursement workflow definition + wire the claim to the engine) — confirm at kickoff.**
...
- **Phase:** **Stage C IN PROGRESS** — **workflow engine (R-4 core) ✅ + R-1 reimbursement
  model+config ✅**. Stage A (`phase-0-complete`) + Stage B (`phase-2-complete`) remain
  complete + pushed. **Migration head now `0013`** (R-1: 13 `reimb_*` tables +
  `core_reference_sequences`). **Verified: pytest 340/340 (+20 this increment), lint-imports
  3/3, migration `0012↔0013` idempotent + reversible.** **NOT pushed** — Stage C pushes at
  its QA gate (reimbursement sub-phases tag `reimb-R<N>-complete`; cadence to confirm).
  Master Plan v1 in force.
- **Last session:** #12 — 2026-07-27 — **Stage C R-1: reimbursement model + config pack**.
  13 `reimb_*` tables (migration `0013`, autogenerated then hand-tuned: append-only REVOKE
  UPDATE + enum-drop-on-downgrade) — claims (+ `workflow_instance_id` FK INTO the engine),
  itinerary legs, cash advances (+ **CA hard-block** partial-unique constraint), EO 77
  3-cluster DTE + PSGC region map (effective-dated), config pack, checklist catalog+items,
  return taxonomy + append-only return/status/external logs, core-attachments join. Ships
  the **core reference-number service #5** (`core_reference_sequences` + `allocate_reference_number`,
  `RB-`/`LQ-YYYY-NNNN`). Regulatory reference data seeded (`modules/reimbursement/seeds.py`).
  5 new test files (+ extended gate tests). **No new dependency.**
- **Decisions this session:** first Stage-C increment was the engine (#11); this increment
  is R-1. **Dropped `reimb_approval_steps`** (= `core_workflow_steps`), **deferred
  `reimb_signatory_configs` → R-4-app** (= the workflow definition) + **`reimb_template_maps`
  → R-5** (Rule 10 — no engine duplication); **`destination_class` → `destination_region_code`**
  (PSGC) routed via `reimb_region_clusters`/`reimb_dte_clusters`; **CA hard-block = a DB
  partial-unique index** (PD 1445 §89), not a workflow guard; **built ref-numbers as core
  service #5** (was unbuilt); append-only reimb logs are REVOKE-UPDATE + **audited** (module
  tables can't join core's `_UNAUDITED`); 30-day clock stored `calendar` (COA 97-002).
  **Fixtures trimmed** to config/catalog seeds — the full synthetic trip set moves to R-2.
  See `docs/modules/reimbursement.md` §5. (Engine decisions from #11: see `foundation.md` §7.)
- **Blockers / waiting on user:** none. **Kickoff choice for next session:** confirm the
  next increment — **R-2 per-diem computation engine** (pure backend, on the seeded clusters
  + config; unblocks the wizard) vs **the React/Vite/Tailwind shell** (FE stack already
  pinned) vs **R-4-app** (author the reimbursement workflow definition + wire the claim's
  `workflow_instance_id` + status sync). All three are unblocked.
...
core workflow engine (#11) + R-1 reimbursement model+config (#12), both green. Available to
build on (in addition to the Stage A/B foundation): the workflow engine core_workflow_*
(office_connect/core/workflow/ — start_instance/execute_action/available_actions/
create_definition/…; contract docs/standards/workflow-standards.md); the reimbursement
schema office_connect/modules/reimbursement/models/ (reimb_claims with workflow_instance_id
FK, itinerary legs, cash advances + CA hard-block, reimb_dte_clusters + reimb_region_clusters
[EO 77 3-cluster], reimb_configs, checklist catalog+items, return taxonomy + append-only
logs, attachments join) + seeds (modules/reimbursement/seeds.py — EO 77 rates, PSGC map,
COA-2023-004 checklist, config pack, return reasons); the core reference-number service
office_connect/core/reference_numbers.py (allocate_reference_number, RB-/LQ-YYYY-NNNN).
Migration head 0013. pytest 340/340, lint-imports 3/3.
Read CLAUDE.md, then docs/modules/reimbursement.md (delta register + R-phase table — R-4
engine core + R-1 DONE), references/Reimbursement_Module_Build_Spec_v1.md §8 (per-diem
computation) for R-2, docs/standards/workflow-standards.md, master-plan.md §2 Stage C,
docs/research/round1/ph-travel-reimbursement-rules.md (EO 77 apportionment/50%-return-day/
50-km/host-deductions), and — if building the React shell — ui-standards.md + the FE pins
below.
Task: continue Stage C with the NEXT increment — CONFIRM at kickoff which:
  (A) R-2 per-diem COMPUTATION engine (pure backend, recommended next): server-compute a
      claim's totals from its itinerary legs + destination region → cluster (reimb_region_
      clusters → reimb_dte_clusters) + reimb_configs — 100% arrival/full days, 50% on the
      departure/return day, 0% inside 50 km w/o overnight, host-provided 50%/30% strips,
      gov-vehicle suppresses fare; write per_diem_pct/per_diem_amount/leg_total + the totals
      JSONB {per_diem, transport, other, grand, advance, to_reimburse, to_refund}. QA: the
      worked example computes ₱5,500 (3-day Manila HUC). Money server-side only.
  (B) React shell (R-2 FE delta) — app shell + 6 layout templates + 13-component inventory
      seed wired to /api/v1/config tokens (Tailwind v4 @theme <- --oc-* vars) + auth/session/
      CSRF (X-Requested-With); add a compose `web` service (Vite :5174) + CORS. FE stack
      CONFIRMED: React 19 + Vite 6 + Tailwind 4 + TypeScript; headless primitives
      (Radix/Headless UI) on the tokens; pick the single icon set + Node version; exact-pin
      package.json (rule 9) + fill ui-standards §7 + tech-stack §4.
  (C) R-4-app — author the reimbursement workflow DEFINITION on the engine (spec §6 status
      machine → core_workflow_states/transitions; certify_A/B/C as enforce_segregation gates;
      amount-tier → required_permission from DOH DO 2019-0225), wire reimb_claims.
      workflow_instance_id at submit + sync the denormalized status, register the escalation
      notifier via core.workflow.register_sla_enqueuer.
...
Open questions for the user (confirm at kickoff): (a) which increment next (R-2 compute /
React shell / R-4-app); (b) if the shell — the single icon library + Node version pin;
(c) push cadence — tag/push reimb-R<N>-complete per increment (development-workflow §4) or
once at the Stage C R-9 gate?
```
...
| C | R-0…R-9 | Reimbursement vertical + core workflow engine + first React shell | in progress (engine core ✅ + R-1 ✅) | 11–12 | — | — |
