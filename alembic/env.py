"""Alembic environment — async engine (run_sync pattern), single chain.

Runs as the OWNER role (``MIGRATION_DATABASE_URL`` → ``oc_dev``), never as the
runtime role ``oc_app`` (which cannot create/alter objects by design).
"""
